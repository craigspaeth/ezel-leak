...
var send = function (method, model, options, url, data, deferred) {
  ...
  req.timeout(2000).end(function(err, res) {
    err ? error(options, err, deferred) : success(options, res, deferred);
  });
}

var success = function(options, res, deferred) {
...
var error = function(options, err, deferred) {
  if (options.error) options.error(err);
  if (options.complete) options.complete(err);
  deferred.rejct(res.body);
}
...
module.exports = function(method, model, options) {
  ...
  if (cached) {
    cacheClient.get(cacheKey, function(err, cachedJSON) {
      if (err) {
        error(options, err, deferred);
      } else if (cachedJSON) {
        success(options, JSON.parse(cachedJSON), deferred);
      } else {
        send(method, model, options, url, data, deferred);
      }
    });
  } else {
    send(method, model, options, url, data, deferred);
