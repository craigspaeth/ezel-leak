...
module.exports = function(method, model, options) {
  ...
  //   // }
  //   request[METHOD_MAP[method]](url).end(function(err, res) {
  ...
  request[METHOD_MAP[method]](url).timeout(2000).end(function(err, res) {
    if (err && options.error) {
      options.error(err);
    } else if (!err && options.success) {
      options.success(res.body);
    }
    if (options.complete) options.complete(err || res.body);
