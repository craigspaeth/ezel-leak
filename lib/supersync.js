...
var fixture = require('../apps/api/fixture');
...
module.exports = function(method, model, options) {
  ...
  //   success(options, { body: fixture() }, function(){})
  // }, 500 + Math.random() * 500);

  // var url = (options.url ||
  //   (typeof model.url == 'function' ? model.url() : model.url));
  // request.get(url).timeout(2000).end(function(err, r) {
  //   if (err || !r.ok) {
  //     error(options, err || r, function(){});
  //   } else {
  //     success(options, r, function(){});
  //   }
  // });
  // return
  ...
  request.get(url).timeout(2000).end(function(err, r) {
    if (err || !r.ok) {
      error(options, err || r, function(err) {
        deferred.reject(err);
        deferred = null;
      });
    } else {
      success(options, r, function(body) {
